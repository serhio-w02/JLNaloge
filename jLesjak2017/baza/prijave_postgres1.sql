CREATE TABLE uporabnik (
  ...
  privilegij smallint NOT NULL DEFAULT 0,
...
create type stattus as enum('Nepregledano','Odobreno','Zavrnjeno');
create type proggram as enum('Tehnik računalništva','Elektrotehnik');

CREATE TABLE vloga (
  vloga_id serial NOT NULL;
  EMSO character varying(13) NOT NULL unique,
  Ime character varying(30)  NOT NULL,
  Priimek character varying(30) NOT NULL,
  Izobrazevalni_program proggram NOT NULL,
  Razred character varying(7)  NOT NULL,
  Naslov_naloge character varying(100)  NOT NULL,
  Naslov_naloge_eng character varying(100)  NOT NULL,
  Opis_naloge character varying(1000) NOT NULL,
  Datum date NOT NULL,
  Podpis_kandidata character varying(30) NOT NULL,
  MentorID integer NOT NULL,
  Odobritev_mentor stattus NOT NULL DEFAULT 'Nepregledano',
  Odobritev_komisija stattus NOT NULL DEFAULT 'Nepregledano',
  CONSTRAINT vlogaid_pkey PRIMARY KEY (vloga_id)
);

ALTER TABLE vloga ADD  FOREIGN KEY(MentorID) REFERENCES mentor(MentorID);

CONSTRAINT article_user_id_fkey FOREIGN KEY (user_id)
      REFERENCES "user" (id) MATCH SIMPLE
      ON UPDATE CASCADE ON DELETE CASCADE

CREATE SEQUENCE vloga_id_seq
  INCREMENT 1
  MINVALUE 1
  MAXVALUE 9223372036854775807
  START 1
...
CREATE TABLE mentor (
  ...
  Ime_mentorja character varying(30) NOT NULL,
  Priimek_mentorja character varying(30) NOT NULL,
  CONSTRAINT mentorid_pkey PRIMARY KEY (id)
);
